...
runner.test('loadModule: unknown path, module-dir', function () {
  a.throws(
    () => loadModule('./adsfdf', { moduleDir: '/some/where/wrong' }),
    err => err.code === 'MODULE_NOT_FOUND'
  )
})
...
runner.test('loadModule: absolute path to a dir', function () {
...
runner.test('loadModule: full module name (module-prefix supplied)', function () {
  const module = loadModule('array-back', { modulePrefix: 'array-' })
  a.strictEqual(module.name, 'arrayify')
})

runner.test('loadModule: relative path to module dir, current dir default', function () {
  const module = loadModule('test/fixture/loadModule/some-module')
  a.strictEqual(module.name, 'someModule')
})

runner.test('loadModule: relative path to module dir, current dir default, moduleDir', function () {
  const module = loadModule('test/fixture/loadModule/some-module', {
